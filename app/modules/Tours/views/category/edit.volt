<form method="post" class="ui form" action="" enctype="multipart/form-data">
    ...
    <div class="ui segment">
        ...
        <a href="{{ url.get() }}tours/category?lang={{ constant('LANG') }}" class="ui button">
        ...
        {% if model.getId() %}
            ...
            <a href="{{ url.get() }}tours/category/delete/{{ model.getId() }}?lang={{ constant('LANG') }}"
               class="ui button red">
            ...
            {% if model.getId() %}
                <a class="ui blue button" target="_blank"
                   href="{{ helper.langUrl(['for':'tours','type':model.getSlug()]) }}">
                    See section on site
                </a>
            {% endif %}
