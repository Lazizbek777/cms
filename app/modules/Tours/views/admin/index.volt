...
<div class="ui segment">
    ...
    <a href="{{ url.get() }}tours/admin/add" class="ui button positive">
        <i class="icon plus"></i> {{ helper.at('Add New') }}
    </a>

    <a href="{{ url.get() }}tours/category" class="ui button">
        <i class="icon list"></i> {{ helper.at('Category') }}
    </a>

    <a href="{{ url.get() }}tours/country" class="ui button">
        <i class="icon list"></i> {{ helper.at('Countries') }}
    </a>
...
<div class="ui tabular menu">
    <a href="{{ url.get() }}tours/admin?lang={{ constant('LANG') }}"
       class="item{% if not category_id %} active{% endif %}">{{ helper.at('All') }}</a>
    {% for category_el in categories %}
        <a href="{{ url(['for':'tours_admin','category':category_el.getSlug()]) }}?lang={{ constant('LANG') }}"
...
{% if paginate.total_items > 0 %}
    ...
    <table class="ui table very compact celled">
        ...
        {% for item in paginate.items %}
            {% set link = url.get() ~ "tours/admin/edit/" ~ item.getId() %}
            {% set image = helper.image(['id':item.getId(),'type':'tours','width':50]) %}
            <tr>
                ...
                {% set url = helper.langUrl(['for':'tour', 'category':item.getCategorySlug(), 'slug':item.getSlug()]) %}
