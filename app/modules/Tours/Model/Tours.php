...
class Tours extends Model
{
    ...
    public function initialize()
    {
        ...
        $this->hasMany('id', 'Tours\Model\Gallery', 'tour_id', [
            'alias' => 'photos'
        ]);

        $this->hasMany('id', 'Tours\Model\Timetable', 'tour_id', [
            'alias' => 'timetable',
            'reusable' => true, 
            'params' => [
        ...
        $this->belongsTo('category_id', 'Tours\Model\Tcategory', 'id', [
            'alias' => 'category'
        ]);
        $this->belongsTo('country_id', 'Tours\Model\Country', 'id', [
            'alias' => 'country'
        ]);
    }
    ...
    protected $translateModel = 'Tours\Model\Translate\ToursTranslate'; // translate
    ...
    public function validation()
    {
        ...
        $validator->add('slug', new UniquenessValidator(
            [
                "model"   => $this,
                "message" => $this->getDi()->get('helper')->translate("Tour with slug is already exists")
            ]
        ));
    ...
    public function updateFields($data)
    {
        ...
        $this->setTopTour(isset($data['top_tour']) ? 1 : 0);
    ...
    {
        $key = HOST_HASH . md5('Tours\Model\Tours; slug = ' . $slug);
        return $key;
    }
    ...
    {
        if ($this->category_id) {
            $category = Tcategory::cachedListArray(['key' => 'id']);
    ...
    {
        if ($this->category_id) {
            $category = Tcategory::cachedListArray(['key' => 'id', 'value' => 'slug']);
            if (array_key_exists($this->category_id, $category)) {
                return $category[$this->category_id];
            }
        }
    ...
    {
        if ($this->country_id) {
            $country = Country::cachedListArray(['key' => 'id']);
            if (array_key_exists($this->country_id, $country)) {
    ...
    {
        if ($this->country_id) {
            $country = Country::cachedListArray(['key' => 'id', 'value' => 'slug']);
            if (array_key_exists($this->country_id, $country)) {
                return $country[$this->country_id];
            }
        }
    ...
    public function setTopTour($top_tour)
    {
        $this->top_tour = $top_tour;
        return $this;
    }

    public function getTopTour()
    {
        return $this->top_tour;
    }
