...
use Tours\Form\ToursForm;
use Tours\Model\Tcategory;
use Tours\Model\Gallery;
use Tours\Model\Timetable;
...
class AdminController extends Controller
{
    ...
    public function initialize()
    {
        ...
        $this->helper->activeMenu()->setActive('admin-tours');
    }
    ...
    public function indexAction()
    {
        ...
        $categories = Tcategory::find();

        $cond_array = [];
        if ($category) {
            $categoryEntity = Tcategory::getCachedBySlug($category);
        ...
        $this->helper->title($this->helper->at('Tours'), true);
    }
    ...
    public function addAction()
    {
            ...
            if ($form->isValid()) {
                if ($model->create()) {
                    ...
                    if ($model->update()) {
                        ...
                        $this->flash->success($this->helper->at('Tour created'));
                        return $this->redirect($this->url->get() . 'tours/admin/edit/' . $model->getId() . '?lang=' . LANG);
                    } else {
        ...
        $this->view->form = $form;
        $this->view->fields = $model->timetable;
        $this->helper->title($this->helper->at('Create a tour'), true);
    ...
    public function editAction($id)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                if ($model->save()) {
                    $this->timetable($model);
                    $this->uploadImage($model);
                    $this->flash->success($this->helper->at('Tour edited'));
                    return $this->redirect($this->url->get() . 'tours/admin/edit/' . $model->getId() . '?lang=' . LANG);
                } else {
                    $this->flashErrors($model);
                }
        ...
        $this->view->fields = $model->timetable;
        $this->helper->title($this->helper->at('Edit tour'), true);
    }
    ...
    public function deleteAction($id)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $model->delete();
            $this->redirect($this->url->get() . 'tours/admin');
        }
    ...
    private function timetable($model)
    {
        if ($this->request->isPost()) { 

            $title_array = $this->request->getPost('title_add'); 
            $day_array = $this->request->getPost('day_add');     
            $text_array = $this->request->getPost('text_add');     
            ...
            foreach ($title_array as $key => $title) {
                ...
                $id = $id_array[$key];
                if($id != 0){
                    $addModel = Timetable::findFirst($id);
                    $addModel->setTitle($title);
                    $addModel->setText($text);                
                    $addModel->setDay($day);                
                    $addModel->update();
                } else {
                    $addModel = new Timetable();
                    $addModel->setTourId($model->getId());
                    $addModel->setTitle($title);
                    $addModel->setText($text);
                    $addModel->setDay($day);                
